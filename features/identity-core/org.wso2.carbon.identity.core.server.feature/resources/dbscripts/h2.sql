...
('8ec6dbf1-218a-49bf-bc34-babed3b585df', 'EKYC_CONFIGURATION', 'A resource type to keep the EKYC configurations'),
...
CREATE TABLE IF NOT EXISTS IDN_USER_EKYC_VC (
    ID INTEGER NOT NULL AUTO_INCREMENT,
    SESSION_ID VARCHAR(255) NOT NULL,
    USER_ID VARCHAR(255) NOT NULL,
    TENANT_ID INTEGER NOT NULL DEFAULT -1,
    ...
    VC CLOB,
    PRIMARY KEY (ID),
    CONSTRAINT IDN_USER_EKYC_VC_CONSTRAINT UNIQUE (USER_ID, TENANT_ID, SESSION_ID)
);
...
-- IDN_EKYC_SESSION --
CREATE INDEX IDX_EKYC_SESSION ON IDN_USER_EKYC_VC (SESSION_ID,USER_IS,TENANT_ID);
